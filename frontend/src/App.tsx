...
function App() {
  ...
  const [filters, setFilters] = useState({
    ...
    region: '',
    language: '',
    min_trend_score: 0,  // 트렌드 점수 필터 제거 (0으로 설정)
    sort_by: 'trend_score',
    video_type: '',  // 쇼츠/롱폼 필터
    time_filter: 'all'  // 기간 필터
  });
  ...
  const loadTrendingVideos = async (reset = true, forceRefresh = false) => {
    console.log('🔄 데이터 로드 시작:', { reset, forceRefresh, filters });
  ...
  const handleVideoSelect = (video: TrendingVideo) => {
    setSelectedKeyword(video.keywords[0]);
  };
                ...
                {trendingVideos.map((video, idx) => (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {video.keywords.slice(0, 3).map((kw, kidx) => (
                        <span
                          key={kidx}
